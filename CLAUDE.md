...
- `landing/index.html` — public marketing landing (this is its own deploy root). Standalone static page, no Supabase/backend dependency at all — WhatsApp ordering only. Product list/prices/WhatsApp number are **placeholder data** in a config block at the top of the file (`WHATSAPP_NUMBER`, `PRODUCTOS`), clearly marked to edit directly — real data pending from the business owner.
